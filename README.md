# GWP 샘플 자동화 (Gift-With-Purchase Automation)

주문 조건(금액 이상 / 특정 상품 구매)을 만족하면 스토어프론트에서 샘플을 장바구니에 자동으로 추가하고, 결제 시 무료로 처리하며, 재고가 소진되면 해당 증정품을 자동으로 비활성화하는 Shopify 앱입니다.

## 동작 방식

1. **관리자 화면** (`/app/promotions`) — Remix + Prisma(SQLite)로 만든 임베드 앱. 프로모션(조건, 배정 방식)과 그에 연결된 샘플 상품(재고 포함)을 관리합니다.
2. **테마 앱 익스텐션** (`extensions/gwp-widget`) — 스토어프론트에 삽입되는 앱 임베드. `assets/gwp.js`가 장바구니 변경을 감지할 때마다 앱 프록시(`/apps/gwp/evaluate`)에 문의해 어떤 프로모션이 조건을 충족했는지 확인하고, `/cart/add.js` · `/cart/change.js`로 증정품 라인을 자동으로 추가/제거합니다. 증정품 라인에는 숨김 속성(`_gwp_free`, `_gwp_promotion_id`)이 붙습니다.
3. **Shopify Function (Discount)** (`extensions/gwp-free-gift-discount`) — `_gwp_free=true` 속성이 붙은 카트 라인을 100% 할인해 실제로 무료로 결제되게 합니다. 앱 설치 시(`afterAuth` 훅, `app/models/setup.server.ts`) 이 함수를 사용하는 자동 할인이 자동으로 생성됩니다.
4. **재고/지표** — 증정품은 실제 Shopify 상품이라 주문 시 Shopify 재고가 그대로 차감됩니다. `inventory_levels/update` 웹훅이 이를 앱 DB(`Gift.stock`)에 반영해 소진 시 자동 비활성화하고, `orders/paid` 웹훅이 GWP 적용 주문을 기록해 대시보드 지표와 "순환 배정" 다음 순번을 갱신합니다.

## 로컬에서 실행하기

이 프로젝트는 `pnpm` 워크스페이스(`pnpm-workspace.yaml`)로 구성되어 있으므로 `pnpm`을 사용하세요.

```shell
npm install -g @shopify/cli@latest pnpm
pnpm install
pnpm exec shopify app dev
```

`shopify app dev`를 처음 실행하면 Partner 계정 로그인과 앱 연결(또는 새 앱 생성), 개발 스토어 선택을 안내합니다. 실행되면:

- `shopify.app.toml`의 `client_id`, 웹훅, 스코프가 자동 반영됩니다.
- `[app_proxy]`의 `url` 값(`https://REPLACE_WITH_APP_URL/proxy`)은 터널 URL에 맞춰 직접 수정하거나 Partner Dashboard의 앱 설정 > App proxy 화면에서 확인/저장해야 할 수 있습니다.
- 개발 스토어의 온라인 스토어 테마 편집기에서 **앱 임베드 > GWP 샘플 자동화**를 켜야 스토어프론트 위젯이 동작합니다.
- 자동 할인(`GWP 무료 증정품`)은 함수 익스텐션이 배포된 뒤 앱을 설치/재인증하면 자동 생성됩니다(수동 설정 불필요).

## 사용 흐름 (머천트)

1. **프로모션** 탭에서 "프로모션 추가" → 조건(금액/특정 상품)과 배정 방식(고정/랜덤/순환) 설정 후 저장.
2. 편집 화면에서 "상품에서 추가"로 샘플 상품(재고 포함)을 1개 이상 연결.
3. 스토어프론트에서 조건을 만족하는 장바구니를 만들면 샘플이 자동으로 추가되고 결제 시 무료로 표시됩니다.
